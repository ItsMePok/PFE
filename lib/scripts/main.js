import { system, world, EquipmentSlot, EntityComponentTypes, ItemComponentTypes, CommandPermissionLevel, CustomCommandParamType, CustomCommandStatus, ItemLockMode } from "@minecraft/server";
import { MinecraftEnchantmentTypes } from "@minecraft/vanilla-data";
import { PFEBossEventConfigName, PFEDefaultBossEventSettings, startBossEvents } from "./bossEvents";
import { PFETimeValidation, PokeTimeGreeting, PokeTimeZoneOffset } from "./time";
import { PFEDisableConfigDefault, PFEDisableConfigName, OpenPFEConfig } from "./config";
import { PFECustomArmorEffectDynamicProperty, startSetEffects } from "./armorEffects";
...
world.afterEvents.playerJoin.subscribe((data => {
    let birthdays = JSON.parse(world.getDynamicProperty(`poke_pfe:birthdays`).toString());
...
world.afterEvents.worldLoad.subscribe((data) => {
    ...
    if (typeof birthdayProperty != "string")
        world.setDynamicProperty(`poke_pfe:birthdays`, `[]`);
    ...
    switch (data.id) {
        ...
        */
        case `poke_pfe:add_set_effect_preset`: {
            const currentPresets = JSON.parse(world.getDynamicProperty(PFECustomArmorEffectDynamicProperty).toString()) ?? [];
        ...
        */
        case PFECustomMineQuestsPropertyID: {
            const currentQuests = JSON.parse(world.getDynamicProperty(PFECustomMineQuestsPropertyID).toString()) ?? [];
        ...
        case PFECustomKillQuestsPropertyID: {
            const currentQuests = JSON.parse(world.getDynamicProperty(PFECustomKillQuestsPropertyID).toString()) ?? [];
            let newQuests = currentQuests.concat(JSON.parse(data.message).value) ?? currentQuests;
            world.setDynamicProperty(PFECustomKillQuestsPropertyID, JSON.stringify(newQuests));
            break;
        }
        case PFECustomFarmQuestsPropertyID: {
            const currentQuests = JSON.parse(world.getDynamicProperty(PFECustomFarmQuestsPropertyID).toString()) ?? [];
            let newQuests = currentQuests.concat(JSON.parse(data.message).value) ?? currentQuests;
            world.setDynamicProperty(PFECustomFarmQuestsPropertyID, JSON.stringify(newQuests));
        }
        case PFECustomCraftQuestsPropertyID: {
            const currentQuests = JSON.parse(world.getDynamicProperty(PFECustomCraftQuestsPropertyID).toString()) ?? [];
            let newQuests = currentQuests.concat(JSON.parse(data.message).value) ?? currentQuests;
