import { Direction, EntityComponentTypes, EquipmentSlot, GameMode, ItemComponentTypes, ItemLockMode, ItemStack, world } from "@minecraft/server";
import { ActionFormData } from "@minecraft/server-ui";
import { MinecraftEnchantmentTypes, MinecraftEntityTypes } from "@minecraft/vanilla-data";
export { PokeDamageItemUB, PokeDamageItem, PokeDecrementStack, PokeErrorScreen, PokeGetItemFromInventory, PokeItemTranslateString, PokeGetObjectById, PokeSaveProperty, PokeGetSegmentOfString, PokeSpawnLootTable, PokeClosestCardinal, getComponentInfoFromDataStorageItems, pokeAddItemsToPlayerOrDrop };
...
function PokeDamageItemUB(item, multiplier, entity, slot, preventBreaking) {
    ...
    if (preventBreaking && (durabilityComponent === null || durabilityComponent === void 0 ? void 0 : durabilityComponent.maxDurability) == (durabilityComponent.damage + 1)) {
    ...
    var unbreakingL = (_b = (_a = enchantableComponent === null || enchantableComponent === void 0 ? void 0 : enchantableComponent.getEnchantment(MinecraftEnchantmentTypes.Unbreaking)) === null || _a === void 0 ? void 0 : _a.level) !== null && _b !== void 0 ? _b : 0;
    if (!slot) {
    ...
    let damage = Number(Math.round(Math.random() * 100) <= durabilityComponent.getDamageChance(unbreakingL)) * (multiplier !== null && multiplier !== void 0 ? multiplier : 1);
    ...
    // Apply damage
    if (durabilityComponent.damage >= durabilityComponent.maxDurability) {
        if (((_c = equippableComponent.getEquipment(slot)) === null || _c === void 0 ? void 0 : _c.typeId) == item.typeId) {
...
function PokeSaveProperty(propertyId, item, save, entity, slot) {
    ...
    if (typeof slot == "number") {
        const slottedItem = (_a = entity.getComponent(EntityComponentTypes.Inventory)) === null || _a === void 0 ? void 0 : _a.container.getSlot(slot);
        slottedItem === null || slottedItem === void 0 ? void 0 : slottedItem.setItem(item);
    }
    else {
        if (!slot)
            slot = EquipmentSlot.Mainhand;
        if ((equippableComponent === null || equippableComponent === void 0 ? void 0 : equippableComponent.getEquipmentSlot(slot).typeId) == item.typeId) {
 ...
 */
function PokeGetSegmentOfString(string) {
    let returnedValue = "{";
...
function getComponentInfoFromDataStorageItems(dynamicPropertyId, componentIdentifier) {
    var _a;
    const dynamicPropertyInfo = world.getDynamicProperty(dynamicPropertyId);
    const itemIds = (_a = JSON.parse(typeof dynamicPropertyInfo == "string" ? dynamicPropertyInfo : "[]")) !== null && _a !== void 0 ? _a : [];
