...
class ComputersCompat {
    ...
    static addStat(statID, amount) {
        var _b;
        (_b = this.addonStatObjective) === null || _b === void 0 ? void 0 : _b.addScore(`${this.addonNameSpace}:${statID}`, amount);
    }
}
_a = ComputersCompat;
//set your namespace
ComputersCompat.addonNameSpace = "poke_pfe";
//leave the rest!
ComputersCompat.computersScoreboardID = "jig_computer.addon_stats";
ComputersCompat.addonStatScoreboardID = `${_a.addonNameSpace}:${_a.computersScoreboardID}`;
export default ComputersCompat;
...
export const initExampleStickers = () => {
    ...
    entityChunks.forEach((entityChunk, index) => {
        ...
        system.runTimeout(() => void world
            ...
            .runCommandAsync(`scriptevent mineapi:jig_pcs ${JSON.stringify(m)}`), delay);
    });
