...
function ViewUpgradeableItems(component, player, block) {
    ...
    UI.title({ translate: component.name });
    let validItems = [];
    for (const item of (_a = PokeGetItemFromInventory(player, undefined, undefined, true)) !== null && _a !== void 0 ? _a : []) {
        const upgradeableComponent = (_b = item.item.getComponent(PFEUpgradeableId)) === null || _b === void 0 ? void 0 : _b.customComponentParameters.params;
        if (!upgradeableComponent)
            continue;
        UI.button({ translate: MakeLocalizationKey(item.item.typeId) }, getTexturePathByIdentifier((_c = item.item.typeId) !== null && _c !== void 0 ? _c : "undefined"));
    ...
    UI.show(player).then(response => {
        ...
        for (const item of validItems) {
            if (response.selection == selection) {
                ...
                ;
                const upgradeableComponent = (_a = ItemStack.getComponent(PFEUpgradeableId)) === null || _a === void 0 ? void 0 : _a.customComponentParameters.params;
