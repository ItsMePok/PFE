 ...
 */
import { BlockPermutation, BlockTypes, ItemComponentTypes, ItemStack } from "@minecraft/server";
...
import { ParsePFEUpgradeComponent, PFEUpgradeableId, PokeUpgradeUI } from "./upgrades";
...
function ViewUpgradeableItems(component, player, block) {
    ...
    UI.title({ translate: component.name });
    let validItems = [];
    for (const item of PokeGetItemFromInventory(player, undefined, undefined, true) ?? []) {
    ...
    UI.show(player).then(response => {
        ...
        for (const item of validItems) {
            if (response.selection == selection) {
                ...
                ;
                const upgradeableComponent = ItemStack.getComponent(PFEUpgradeableId)?.customComponentParameters.params;
                ...
                for (const result of ParseRecipeItems(recipe.result)) {
                    ...
                    for (let i = result.amount; i > -1; i = i - maxAmount) {
                        ...
                        if (recipe.recipe_type !== "transform") {
                            ...
                            let resultedItem = new ItemStack(result.item, clampNumber(i, 0, maxAmount));
                            if (typeof recipe.keep_item_data_from === "number") {
                                let dataSendingItem = PokeGetItemFromInventory(player, undefined, requiredItems.at(recipe.keep_item_data_from)?.item)?.at(0);
                                if (dataSendingItem) {
                                    ...
                                    console.warn(`Failed to find item to transfer data from. Looked for ${JSON.stringify(dataSendingItem)} || PFE - RecipeBlockComponent - TransformRecipeType`);
                            }
