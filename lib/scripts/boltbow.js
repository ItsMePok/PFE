...
function PFEAmmoManagementMainMenuUI(item, player) {
    ...
    UI.button({ translate: `translation.poke:ammoUIQuickReload`, with: { rawtext: [{ text: `${boltBowComponent.projectile.amount}` }, { text: `${((_b = (_a = boltBowComponent.upgrades.filter(upgrade => upgrade.id == CapacityUpgradeDefault.id).at(0)) === null || _a === void 0 ? void 0 : _a.level) !== null && _b !== void 0 ? _b : 1) * 16}` }] } }, `textures/poke/common/ammoQuickReload`);
    ...
    UI.show(player).then((response => {
        ...
        if (response.selection == selection) { // Upgrade
            PokeUpgradeUI(player, item, boltBowComponent, PFEAmmoManagementMainMenuUI(item, player), false, (_b = (_a = item.getComponent(PFEUpgradeableId)) === null || _a === void 0 ? void 0 : _a.customComponentParameters.params) !== null && _b !== void 0 ? _b : undefined);
            return;
        }
        else
            selection++;
