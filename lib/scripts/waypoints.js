...
function WaypointUIMainMenu(player, itemStack, component, reParseItem) {
    ...
    let item = itemStack;
    if (reParseItem) {
        item = (_b = (_a = player.getComponent(EntityComponentTypes.Equippable)) === null || _a === void 0 ? void 0 : _a.getEquipment(EquipmentSlot.Mainhand)) !== null && _b !== void 0 ? _b : item;
    }
    let UI = new ActionFormData();
    let waypointConfig = (_c = JSON.parse(item.getDynamicProperty(WaypointDynamicPropertyID).toString())) !== null && _c !== void 0 ? _c : PFEWaypointDefaultConfig;
    UI.title({ translate: `poke_pfe.waypoint_menu` });
    UI.button({ translate: `%poke_pfe.waypoints [${(_d = waypointConfig.waypoints.length) !== null && _d !== void 0 ? _d : 0}/${(_e = component.max_waypoints) !== null && _e !== void 0 ? _e : WaypointTotal}]` }, `textures/poke/common/waypointRed`);
    UI.button({ translate: `%poke_pfe.options` }, `textures/poke/common/gear`);
    const UpgradeableComponent = (_f = item.getComponent("poke_pfe:upgradeable")) === null || _f === void 0 ? void 0 : _f.customComponentParameters.params;
...
function WaypointUIOptions(player, item, component, waypointConfig) {
    ...
    let UI = new ActionFormData();
    UI.title({ translate: `%poke_pfe.waypoint_menu` });
    waypointConfig.localSettings.locked ? UI.button({ translate: `%poke_pfe.unlockWaypoints\n%poke_pfe.lockedBy: ${(_a = waypointConfig.localSettings.locker) === null || _a === void 0 ? void 0 : _a.name}` }, "textures/poke/common/locked") : UI.button({ translate: `%poke_pfe.lockWaypoints` }, "textures/poke/common/unlocked");
...
function WaypointUIManageWaypoint(player, item, waypoint, component) {
    ...
    UI.show(player).then(response => {
        ...
        if (response.selection == selection) {
            ...
            if (waypointConfig.localSettings.locked && ((_a = waypointConfig.localSettings.locker) === null || _a === void 0 ? void 0 : _a.id) != player.id) {
                PokeErrorScreen(player, { text: `This waypoint was locked by ${(_c = (_b = waypointConfig.localSettings.locker) === null || _b === void 0 ? void 0 : _b.name) !== null && _c !== void 0 ? _c : "someone"}, this need to be unlocked by the locker in options to teleport to this waypoint again` }, WaypointUIManageWaypoint(player, item, waypoint, component));
                return;
            }
