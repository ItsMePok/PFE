...
import { PFEDisableConfigDefault, PFEDisableConfigName } from "./config";
...
function CheckEffects(player, additionalOptions, customParse) {
    ...
    for (let effect of effects) {
        ...
        if (!ActiveEffects) {
            ...
            player.addEffect(effect.effect, effectDuration, {
                showParticles: false,
                amplifier: clampNumber(Math.min((ActiveEffects.amplifier + 1), totalPieces, effect.max_amp, clampNumber(CurrentEffect - 1, 0, 255)), 0, 25)
            });
        }
    }
    ...
    for (let radiusEffect of compiledRadiusEffects) {
        ...
        const targets = player.dimension.getPlayers({
            ...
            maxDistance: clampNumber(radiusEffect.totalRadius ?? radiusEffect.radius_per_piece ?? radiusEffect.max_radius, 0, radiusEffect.max_radius),
            excludeNames: radiusEffect.effect_self ? undefined : [player.name]
        });
        for (let target of targets) {
            target.addEffect(radiusEffect.effect, effectDuration, {
                showParticles: false,
                amplifier: clampNumber(radiusEffect.totalAmp ?? radiusEffect.amp ?? 0, 0, radiusEffect.max_amp)
            });
        }
    }
}
