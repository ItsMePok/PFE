...
export { PFEDisableConfigName, PFEDisableConfigDefault, PFEDisableConfigMainMenu, PFEDisabledOnUseItems, IsPFEFeatureEnabled, OpenPFEConfig };
    ...
    UI.show(player).then(response => {
        ...
        if (response.selection == selection) { // Cactus Armor's Radius Effects
            if (newProperty.cactusArmorRadius) {
                ...
                newProperty.cassetteTrader = true;
            world.setDynamicProperty(PFEDisableConfigName, JSON.stringify(newProperty));
            PFEDisableConfigMainMenu(player);
            return;
        }
...
function IsPFEFeatureEnabled(feature) {
    const DisabledFeatures = JSON.parse(world.getDynamicProperty(PFEDisableConfigName)?.toString() ?? JSON.stringify(PFEDisableConfigDefault));
    ...
    else if (DisabledFeatures[feature] === true)
        return true;
    else
        return false;
}
