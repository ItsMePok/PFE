...
*/
export { PFEQuestPropertyID, PFEQuestVersion, PFERollQuest, PokePFEQuestComponent, PFECustomMineQuestsPropertyID, PFECustomFarmQuestsPropertyID, PFECustomCraftQuestsPropertyID, PFECustomKillQuestsPropertyID };
...
function PFERollQuest(item, player, questType) {
    ...
    let AddingQuest = (_b = (_a = questType.at(Math.round(Math.random() * questType.length))) !== null && _a !== void 0 ? _a : questType.at(0)) !== null && _b !== void 0 ? _b : { requiredItem: {}, reward: { tokenAmount: 0, item: new ItemStack(MinecraftItemTypes.Dirt, 1) } };
...
class PokePFEQuestComponent {
    onUse(data, component) {
        ...
        if (!data.itemStack.getDynamicProperty(PFEQuestPropertyID)) {
            ...
            (additionalComponentInfo === null || additionalComponentInfo === void 0 ? void 0 : additionalComponentInfo.custom_quest_dynamic_property) ? typeof data.itemStack.getDynamicProperty(additionalComponentInfo.custom_quest_dynamic_property) === "string" ? quests = (_a = quests.concat(JSON.parse(data.itemStack.getDynamicProperty(additionalComponentInfo.custom_quest_dynamic_property).toString()))) !== null && _a !== void 0 ? _a : quests : undefined : undefined;
...
function QuestInfoUI(item, player) {
    ...
    let quest = (_a = JSON.parse(item.getDynamicProperty(PFEQuestPropertyID).toString())) !== null && _a !== void 0 ? _a : console.warn(`Quest not found or failed to parse || poke_pfe:quest`);
    let validRequiredItems = (_b = PokeGetItemFromInventory(player, undefined, quest.item)) !== null && _b !== void 0 ? _b : false;
    let totalItems = 0;
    ...
    UI.show(player).then(response => {
        ...
        if ((response.selection == selection) && canComplete) {
            ...
            (_a = player.getComponent(EntityComponentTypes.Equippable)) === null || _a === void 0 ? void 0 : _a.setEquipment(EquipmentSlot.Mainhand, undefined);
...
function ConvertQuestV1toV2(quests) {
    ...
    for (let quest of quests) {
        if (!((_a = quest.requiredItem) === null || _a === void 0 ? void 0 : _a.item)) {
