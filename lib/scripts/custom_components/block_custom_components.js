import { BlockComponentTypes, BlockPermutation, BlockVolume, Direction, EntityComponentTypes, EquipmentSlot, GameMode, ItemComponentTypes, ItemStack } from "@minecraft/server";
...
function RegisterBlockComponents(data) {
    data.blockComponentRegistry.registerCustomComponent("poke_pfe:spawn_particle", {
        onRedstoneUpdate(data, componentInfo) {
            ...
            for (const PARTICLE_ID of Object.keys(COMPONENT)) {
                ...
                if (PARTICLE.requires_redstone_power && (data.block.getRedstonePower() ?? 0 > 0))
                    continue;
    ...
    data.blockComponentRegistry.registerCustomComponent("poke_pfe:cycle_color", {
        onPlayerInteract(data, componentInfo) {
            ...
            const LIGHT_COLOR = data.block.permutation.getState(COLOR_STATE);
            ...
            //Adds 1 to the current state of pfe:color
                ...
                data.block.setPermutation(data.block.permutation.withState(COLOR_STATE, LIGHT_COLOR + 1));
                //play sound
                data.block.dimension.runCommand(`playsound block.copper_bulb.turn_on @a ${BLOCK_LOCATION} 1 ${SOUND_PITCH}`);
                ComputersCompat.addStat(`bulb_color_changes`, 1);
                return;
