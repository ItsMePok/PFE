        ...
        onHitEntity(data, component) {
            switch (data.itemStack?.typeId) {
                ...
                case 'poke_pfe:void_scythe': {
                    data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 200, { amplifier: 6 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Wither, 200, { amplifier: 2 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Darkness, 200, { amplifier: 0 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Hunger, 300, { amplifier: 3 });
                    ;
                    return;
                }
                case 'poke_pfe:death_scythe': {
                    data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 200, { amplifier: 6 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Wither, 300, { amplifier: 3 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Darkness, 200, { amplifier: 0 });
                    data.hitEntity.addEffect(MinecraftEffectTypes.Hunger, 300, { amplifier: 4 });
                    return;
                }
                case 'poke_pfe:nebula_scythe': {
                    data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 100, { amplifier: 9 });
                    data.attackingEntity.addEffect(MinecraftEffectTypes.Saturation, 1000, { amplifier: 9 });
                    data.attackingEntity.addEffect(MinecraftEffectTypes.Resistance, 100, { amplifier: 3 });
                ...
                case 'poke_pfe:circuit_sword': {
                    data.attackingEntity.addEffect(MinecraftEffectTypes.FireResistance, 20, { amplifier: 0 });
                    data.attackingEntity.runCommand('execute as @e[rm=0.1,r=7,type=!lightning_bolt] at @s run summon lightning_bolt ~~~');
    ...
    data.itemComponentRegistry.registerCustomComponent("poke_pfe:run_command", {
        ...
        onUseOn(data, component) {
            ...
            if (player.typeId != MinecraftEntityTypes.Player)
                return;
            if (typeof componentInfo.command == "string")
                data.block.dimension.runCommand(`execute positioned ${data.block.x} ${data.block.y} ${data.block.z} run ${componentInfo.command}`);
            else if (componentInfo.command)
                for (const command of componentInfo.command) {
