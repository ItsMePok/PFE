...
export { PFEHourTimeDownEvents, PFETimeValidation };
...
function PokeTimeCheck(event, player, claimCheck) {
    ...
    for (let i = event.dates.length; i > -1; i--) {
        ...
        if (claimCheck) {
            if (!event.gift)
                continue;
            if (player.hasTag(`poke_pfe:${currentTime.getFullYear()}E-${event.id}`))
                continue;
        }
        return true;
    }
...
function PokeTimeDeleteEvent(player, event) {
    ...
    UI.show(player).then((response => {
        if (response.canceled) {
            ...
            let customEvents = JSON.parse(world.getDynamicProperty(PokeCustomEventId).toString() ?? "[]");
