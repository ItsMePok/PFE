import { BlockPermutation, BlockStateArg, Direction, EntityComponentTypes, EquipmentSlot, GameMode, ItemComponentTypes, ItemStack, Player, StartupEvent, system, Vector3, world } from "@minecraft/server"
import { PokeTimeConfigUIMainMenu } from "../time"
import { BlockStateSuperset, MinecraftDimensionTypes, MinecraftEffectTypes, MinecraftEntityTypes } from "@minecraft/vanilla-data"
import { PFEBoltBowsComponent } from "../boltbow"
import { OpenPFEConfig, PFEDisableConfigDefault, PFEDisableConfigName, PFEDisableConfigOptions } from "../config"
...
function RegisterItemComponents(data: StartupEvent) {
  ...
  data.itemComponentRegistry.registerCustomComponent(
    ...
    onHitEntity(data, component) {
      switch (data.itemStack?.typeId) {
        ...
        case 'poke_pfe:hellish_sword': { data.hitEntity.addEffect('slowness', 40, { amplifier: 3 }); return; }
        ...
        case 'poke_pfe:creedite_scythe': { data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 100, { amplifier: 4 }); data.hitEntity.addEffect('blindness', 20); return; }
        ...
        case 'poke_pfe:galaxy_scythe': { data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 100, { amplifier: 9 }); data.attackingEntity.addEffect(MinecraftEffectTypes.FireResistance, 300); data.hitEntity.addEffect('wither', 80, { amplifier: 2 }); data.hitEntity.addEffect('weakness', 80, { amplifier: 2 }); return; }
        ...
        case 'poke_pfe:void_scythe': { data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 200, { amplifier: 6 }); data.hitEntity.addEffect(MinecraftEffectTypes.Wither, 200, { amplifier: 2 }); data.hitEntity.addEffect(MinecraftEffectTypes.Darkness, 200, { amplifier: 0 }); data.hitEntity.addEffect(MinecraftEffectTypes.Hunger, 300, { amplifier: 3 });; return; }
        case 'poke_pfe:death_scythe': { data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 200, { amplifier: 6 }); data.hitEntity.addEffect(MinecraftEffectTypes.Wither, 300, { amplifier: 3 }); data.hitEntity.addEffect(MinecraftEffectTypes.Darkness, 200, { amplifier: 0 }); data.hitEntity.addEffect(MinecraftEffectTypes.Hunger, 300, { amplifier: 4 }); return; }
        case 'poke_pfe:nebula_scythe': { data.attackingEntity.addEffect(MinecraftEffectTypes.Speed, 100, { amplifier: 9 }); data.attackingEntity.addEffect(MinecraftEffectTypes.Saturation, 1000, { amplifier: 9 }); data.attackingEntity.addEffect(MinecraftEffectTypes.Resistance, 100, { amplifier: 3 }); data.hitEntity.addEffect('wither', 80, { amplifier: 3 }); return; }
        ...
        case 'poke_pfe:circuit_sword': { data.attackingEntity.addEffect(MinecraftEffectTypes.FireResistance, 20, { amplifier: 0 }); data.attackingEntity.runCommand('execute as @e[rm=0.1,r=7,type=!lightning_bolt] at @s run summon lightning_bolt ~~~'); data.hitEntity.addEffect('blindness', 100); return; }
      }
      return;
    }
  }
