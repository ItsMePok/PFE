  ...
  data.blockComponentRegistry.registerCustomComponent(
    ...
    onTick(data, componentInfo) {
      ...
      type SpawnItemComponent = {
        ...
        spawn_item_at: Vector3
        allow_deposit_to_container: boolean
        allow_spawn_as_dropped_item: boolean
      }
      if (component.needs_state) {
        if (!data.block.permutation.getState(component.needs_state.name) == component.needs_state.value) return
      }
      ...
      pokeAddItemsToContainerOrDrop(
        (component.allow_deposit_to_container ? spawnAtBlock.getComponent(BlockComponentTypes.Inventory)?.container : undefined)
        , spawningItem, data.block.dimension, spawnLocation)
