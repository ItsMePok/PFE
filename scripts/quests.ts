import { CustomComponentParameters, EntityComponentTypes, EquipmentSlot, ItemComponentUseEvent, ItemStack, Player, system, world } from "@minecraft/server";
...
world.afterEvents.worldLoad.subscribe(data => {
  system.runTimeout(() => {
    const AddedMineQuests: PFECustomQuests = {
      value: [
        { requiredItem: { item: `poke_pfe:cut_cobalt`, amount: 7, translationString: `%poke_pfe.cut_cobalt (%poke_pfe.tag)` }, reward: { tokenAmount: 7 } }
      ]
    }
...
function QuestInfoUI(item: ItemStack, player: Player) {
  ...
  UI.show(player).then(response => {
    ...
    if ((response.selection == selection) && canComplete) {
      ...
      player.dimension.spawnItem(new ItemStack(`poke_pfe:copper_token`, quest.tokenAmount), player.location)
