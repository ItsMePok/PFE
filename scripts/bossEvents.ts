...
function PFEBossEventUISettings(player: Player) {
  ...
  new ActionFormData().title({ translate: `translation.poke:bossEventMainMenuTitle` })
    ...
    .button({ translate: `translation.poke:bossEventGoBack` }, 'textures/poke/common/left_arrow')
    .show(player).then((ui: ActionFormResponse) => {
      if (ui.canceled || ui.selection == 3) {
...
function PFEStartBossEvent() {
  ...
  let weights: number[] = [zombkenWeight, superStrikerWeight, knightlingWeight, miniDemonicAllayWeight, necromancerWeight, snowmanWeight, furnaceGolemWeight, sparkyWeight, theLoggerWeight, listenerWeight]
  ...
  const weightedRandom = (array: string[], weights: number[]) => {
